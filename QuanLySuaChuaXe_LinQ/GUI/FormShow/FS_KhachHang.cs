    ...
    public partial class FS_KhachHang : Form
    {
        ...
        private void btn_Xoa_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                DataTable dataTable_Xe = bll_Xe.TimXe_KhachHang(id_KhachHang);

                for (int i = 0; i < dataTable_Xe.Rows.Count; ++i)
                {
                    DataTable dataTable_HopDong = bll_HopDong.TimHopDong_Xe(
                        dataTable_Xe.Rows[i]["ID_Xe"].ToString());

                    for (int j = 0; j < dataTable_HopDong.Rows.Count; ++j)
                    {
                        DataTable dataTable_ChiTietHopDong = bll_ChiTietHopDong.TimChiTietHopDong("", "",
                        ...
                        DataTable dataTable_PhieuThu = bll_PhieuThu.TimPhieuThu_HopDong(
                            dataTable_HopDong.Rows[j]["ID_HopDong"].ToString());
